...
Given a departure city, an arrival city, a flight duration in hours,
and a direction of travel, return the number of jet lag hours the
traveller is experiencing.
...
function getJetLagHours(departureCity, arrivalCity, flightDuration, direction) {
    ...
    const diff = Math.abs(offset[departureCity] - offset[arrivalCity]);

    const dir_mult = direction === "east" ? 1.5 : 1.0;

    const jet_lag_hours = diff + (flightDuration * 0.1) * dir_mult;

    return Math.round(jet_lag_hours * 10) / 10;
}
